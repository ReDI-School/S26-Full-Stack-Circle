...
export const mockEventsResponse: { events: BackendEvent[] } = {
  events: [
    {
    ...
    },
    {
      ...
      description:
        'A hands-on workshop about building scalable design systems for modern web apps.',
...
const isSameUser = (attendee: Attendee, user: MockUser) =>
...
const isPastEvent = (event: BackendEvent) => {
  return new Date(event.date) < new Date();
};

const isUserAttending = (eventId: string, user: MockUser) => {
  return getAttendeesForEvent(eventId).some((attendee) => isSameUser(attendee, user));
};
...
export const getMockProfileEvents = () => {
  const events = mockEventsResponse.events;

  const created = events
    .filter((event) => event.organizerId === mockProfileUser.id)
    .filter((event) => !isPastEvent(event))
    .map((event) => toProfileEvent(event, 'created'));

  const going = events
    .filter((event) => event.organizerId !== mockProfileUser.id)
    .filter((event) => !isPastEvent(event))
    .filter((event) => isUserAttending(event.id, mockProfileUser))
    .map((event) => toProfileEvent(event, 'going'));

  const archived = events
    .filter((event) => isPastEvent(event))
    .filter((event) => {
      const createdByUser = event.organizerId === mockProfileUser.id;
      const attendedByUser = isUserAttending(event.id, mockProfileUser);

      return createdByUser || attendedByUser;
    })
    .map((event) => toProfileEvent(event, 'archived'));
